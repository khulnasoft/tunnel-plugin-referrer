...
func main() {
	...
	rootCmd := &cobra.Command{
		Short: "A Tunnel plugin for oci referrers",
	...
	putCmd := &cobra.Command{
		...
		Example: `  tunnel image -q -f cyclonedx YOUR_IMAGE | tunnel referrer put
  # Put SBOM attestation
  tunnel referrer put -f sbom.json`,
	...
	getCmd := &cobra.Command{
		...
		Example: `  $ tunnel referrer get --type cyclonedx YOUR_IMAGE
  $ tunnel referrer get --digest DIGEST YOUR_IMAGE`,
		RunE: func(cmd *cobra.Command, args []string) error {
	...
	listCmd := &cobra.Command{
		...
		Example: `  $ tunnel referrer list YOUR_IMAGE`,
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			output := viper.GetString("output")
			var writer io.Writer
	...
	treeCmd := &cobra.Command{
		...
		Example: `  $ tunnel referrer tree YOUR_IMAGE`,
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			output := viper.GetString("output")
			var writer io.Writer
